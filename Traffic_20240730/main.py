...
class Solution:
    ...
    def update_objective_value(self, add_path, sumocfg_path):
        obj = ObjectValue('final.net.xml', "final.rou.xml", add_path, sumocfg_path, "tripinfo.xml", "fcd.xml")
...
def GWGO(N, K, phi, max_iter=7500, lower_bound=5, upper_bound=60, f=0.5, l=1.5, c_min=0.00001, c_max=1.0):
    # 1: Initialization
    solutions = [Solution(lower_bound, upper_bound, K) for i in range(N)]
    for i, si in enumerate(solutions):
        ...
        add_file_name = encode_add_xml(si.position, i+1)
        print("a", end="")
        sumocfg_file_name = encode_sumocfg(add_file_name, i+1)
        print("b", end="")
        simulationStart(sumocfg_file_name)
        print("c", end="")
        si.update_objective_value(add_file_name, sumocfg_file_name)
        print("d")
    ...
    for t in range(max_iter):
        ...
        for i, si in enumerate(solutions):
            ...
            # Calculate the objective value using tripinfos.xml and Eq.(1)
            si.position = [int(round(num)) for num in si.position]
            for j in range(len(si.position)):
                if si.position[j] < lower_bound:
                    si.position[j] = lower_bound
                elif si.position[j] > upper_bound:
                    si.position[j] = upper_bound
            add_file_name = encode_add_xml(si.position, i+1)
            sumocfg_file_name = encode_sumocfg(add_file_name, i+1)
            simulationStart(sumocfg_file_name)
            si.update_objective_value(add_file_name, sumocfg_file_name)
