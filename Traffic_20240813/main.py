...
def GWGO(N, phase, phi, max_iter=7500, lower_bound=5, upper_bound=60, f=0.5, l=1.5, c_min=0.00001, c_max=1.0):
    ...
    for t in range(max_iter):
        ...
        for i, si in enumerate(solutions):
            ...
            p = Hunting(s_alpha, s_beta, s_delta, A1, A2, A3, C1, C2, C3, si, N, phase)

            # grasshopper (GO)
            xi = Social_Interaction(i, solutions, si, c, N, phase, lower_bound, upper_bound, f, l)
...
def GWGO_run(N, phase, phi, max_iter=7500, lower_bound=5, upper_bound=60, f=0.5, l=1.5, c_min=0.00001, c_max=1.0):
    ...
    best_solution = GWGO(N=N, phase=phase, phi=phi, max_iter=max_iter, lower_bound=lower_bound, upper_bound=upper_bound, f=f, l=l, c_min=c_min, c_max=c_max)
...
if __name__ == '__main__':
    ...
    phase = 6          # Number of phases a solution has (在這次的路網共有6個phase)
    ...
    GWGO_run(N=N, phase=phase, phi=phi, max_iter=max_iter, lower_bound=lower_bound, upper_bound=upper_bound, f=f, l=l, c_min=c_min, c_max=c_max)