...
class GO():
    ...
    def run(self):
        ...
        for t in range(self.max_iter):
            ...
            for i, si in enumerate(self.solutions):
                # Debugging duration before update
                print(f"Solution {i} before update: {si.duration}")
                
                si.duration = self.Social_Interaction(i, si, c)
                
                # Debugging duration after interaction but before rounding
                print(f"Solution {i} after interaction: {si.duration}")
            ...
            for i, si in enumerate(self.solutions):
                ...
                # Debugging duration after applying bounds
                print(f"Solution {i} after bounding: {si.duration}")
                
                i_sumocfg = encode(i, si.duration)
                simulationStart(HIS_PATH + i_sumocfg, SIM_TIME)
                si.evaluate(i)
                print(f"Solution {i} objective value: {si.value}")  # Debugging objective value
